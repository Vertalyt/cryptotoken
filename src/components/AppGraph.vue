<template>
  <section class="relative">
    <h3 class="text-lg leading-6 font-medium text-gray-900 my-8">{{ coinDataGraph.name }} - USD</h3>
    ...
    <button @click="$emit('rezetDataGraph')" type="button" class="absolute top-0 right-0">
      <svg
...
const normalizedGraph = computed(() => {
  const max = Math.max(...props.graph)
  const min = Math.min(...props.graph)
  const difference = max - min
  if (max === min) {
    return props.graph.map(() => 5)
  }
  return props.graph.map((price) => 5 + ((price - min) * 95) / difference)
})
